...
const Form: React.FC<FormProps> = ({ formType, fields, onSubmit }) => {
  ...
  return (
    <>
      ...
      <main className="min-h-screen flex items-center justify-center bg-blue-50 px-5 pt-28 pb-10">
        <div
          className="bg-white rounded-3xl shadow-xl max-w-5xl w-full overflow-hidden md:flex"
          style={{ minHeight: "600px" }}
        >
          {/* Left image / SVG */}
          <div className="hidden md:flex md:w-1/2 bg-indigo-500 py-12 px-12 items-center justify-center">
          ...
          {/* Form side */}
          <div className="w-full md:w-1/2 px-8 md:px-12 py-12">
            <h1 className="text-center text-3xl font-bold text-gray-900 uppercase mb-3">
              {formType}
            </h1>
              ...
              {fields.map((field, index) => (
                ...
                >
                  ...
                  <div className="relative flex items-center">
                    <i
                      className={`mdi ${field.icon} absolute left-3 text-gray-400 text-lg pointer-events-none`}
                    />
                    <input
                      id={field.name}
                      name={field.name}
