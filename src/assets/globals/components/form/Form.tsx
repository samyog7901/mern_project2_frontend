...
const Form: React.FC<FormProps> = ({ formType, fields, onSubmit }) => {
        ...
        >
          ...
          <div className="w-full md:w-1/2 px-8 md:px-12 py-12 dark:bg-gray-600">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              {fields.map((field, index) => (
                <div
                  ...
                  className={`flex flex-col ${
                    field.fullWidth ? "w-full" : "w-1/2"
                  } ${field.grouped ? "inline-block pr-4" : "block"}`}
                >
                  <label
                    htmlFor={field.name}
                    className="text-xs font-semibold mb-2 block text-gray-700 dark:text-amber-50"
                  >
                    {field.label}
                  </label>

                  <div className="relative flex items-center dark:text-amber-50">
                    <i
                      className={`mdi ${field.icon} absolute left-3 text-gray-400 dark:text-amber-50 text-lg pointer-events-none`}
                    />
                    <input
                      id={field.name}
                      name={field.name}
              ...
              <button
                type="submit"
                className="w-full bg-purple-600 hover:bg-purple-700 dark:bg-gray-700 dark:hover:bg-gray-600 text-white dark:text-white py-3 rounded-lg font-semibold transition hover:cursor-pointer"
              >
                {formType === "Register" ? "REGISTER NOW" : "LOGIN NOW"}
              </button>
              <GoogleLoginButton onClick={handleGoogleLogin} label={formType === "Login" ? "Sign in" : "Sign up"}/>
              ...
              {formType === "Login" ? (
                <>
                  <Link
                    ...
                    className="inline-block w-full sm:w-auto px-4 py-2 rounded-lg text-blue-500 font-medium hover:bg-blue-600 hover:text-white transition-all duration-200 mb-4"
                  >
                    Login as Admin
                  </Link>
                  ...
                  <p>
                    ...
                    <Link
                      to="/register"
                      className="text-purple-600 hover:text-purple-700 dark:text-white dark:hover:text-purple-500 font-semibold transition-all"
                    >
                      Register here
                    </Link>
                  </p>
                ...
                <p>
                  ...
                  <Link
                    to="/login"
                    className="text-purple-600 hover:text-purple-700 dark:text-white font-semibold transition-all"
                  >
                    Login to Dashboard
                  </Link>
                </p>
